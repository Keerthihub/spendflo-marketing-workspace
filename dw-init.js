...
(function () {
  ...
  function loadSigBuilder() {
    ...
    var s = document.createElement('script'); s.defer = true; s.src = '/sig-builder.js?v=30';
  ...
  function loadSocialGallery(){
    if(location.pathname.indexOf('/systems/social-ads')===-1)return;
    if(window.__socialLoaded)return; window.__socialLoaded=true;
    var sc=document.createElement('script'); sc.defer=true; sc.src='/social-gallery.js?v=2'; document.head.appendChild(sc);
  }
  function injectNewUpdatesNav() {
    var nav = document.querySelector('.navpanel'); if (!nav) return;
    var onPage = location.pathname.replace(/\/$/, '').indexOf('/new-updates') !== -1;
    var row = nav.querySelector('a.nu-navrow');
    if (!row) {
      var home = nav.querySelector('a.np-row[href="/"]');
      row = document.createElement('a');
      row.className = 'np-row nu-navrow';
      row.setAttribute('href', '/new-updates/');
      row.innerHTML = '<span class="ic"><svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M10.268 21a2 2 0 0 0 3.464 0"></path><path d="M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326"></path></svg></span><span class="lb">New updates</span><span class="tg tg-new">new</span>';
      if (home && home.parentElement) home.parentElement.insertBefore(row, home.nextSibling);
      else nav.appendChild(row);
    }
    if (onPage) row.classList.add('active'); else row.classList.remove('active');
  }
  /* ---- Email-signature hero: sample person -> Keerthivasan (self-correcting) ---- */
  function patchSigHero() {
    if (location.pathname.indexOf('/systems/email-signature') === -1) return;
    document.querySelectorAll('.content div, .content span').forEach(function (el) {
      if (el.children.length) return;
      var t = el.textContent.trim();
      if (t === 'Anita Joshi') el.textContent = 'Keerthivasan';
      else if (t === 'Head of Procurement \u00b7 Spendflo') el.textContent = 'Visual Designer \u00b7 Spendflo';
      else if (t === '+1 (415) 555-0134') el.textContent = '+91 824-831-4696';
    });
  }
  ...
  function injectHeroFX() {
    ...
    else if (pth.indexOf('/getting-started/introduction') !== -1) { key = 'intro'; html = FX_INTRO; }
    else if (pth.indexOf('/foundations/colour') !== -1) { key = 'colour'; html = FX_COL; }
    else if (pth.indexOf('/foundations/iconography') !== -1) { key = 'icon'; html = FX_ICON; }
    if (!key) return;
